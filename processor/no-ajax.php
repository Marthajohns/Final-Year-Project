...
{  $dest1 = $_POST['origin'];
   $dest2 = $_POST['destination'];
	$route = $dest1."-".$dest2;
 $available = $_POST['travel_date'];

    $sqlroute = "SELECT * FROM route_price where destination1='$dest1' and destination2='$dest2'";
    $resultroute = mysqli_query($conn, $sqlroute);
 $rerou = array();
 $availablebus = array();
  while( $rowrou = mysqli_fetch_assoc($resultroute) ){
     $rerou[] = $rowrou;
  }
 //$_SESSION['route_availability'] = $rerou;
 foreach(array_chunk($rerou, 1) as $entriesRow) {
   foreach ($entriesRow as $entry) {
       ...
       $cash = $entry["cash"];
       $VIPcash = $entry["vip_cash"];
    $sql = "SELECT * FROM seats where date='$available' and route='$route' and busid='$busid'";
 	$result = mysqli_query($conn, $sql);
 	$count = mysqli_num_rows($result);
    $availablebus[] = array("company_code"=>"$company", "vehicle_type"=>"$vtype", "count"=>"$count", "time"=>"$deptime", "cash"=>"$cash", "VIPcash"=>"$VIPcash" , "busid"=>"$busid");
   }
}
 $_SESSION['available'] = $availablebus;
	 ...
	 header('Location: ../listing.php?route='.$route.'&date='.$available);
}
